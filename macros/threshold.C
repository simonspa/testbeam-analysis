...
void threshold(const char* inputdir, int chip, int startrun, int stoprun) {
  ...
  TProfile *resolution = new TProfile("resolution"," ",56,28,84,0,60,"");
  TProfile *resolution_corr = new TProfile("resolution_corr"," ",56,28,84,0,60,"");

  TCanvas *c2 = new TCanvas("c2","resolution",600,600);
  TProfile *resolution_tel_subtracted = new TProfile("resolution_tel_subtracted"," ",56,28,84,0,60,"");
  TProfile *resolution_corr_tel_subtracted = new TProfile("resolution_corr_tel_subtracted"," ",56,28,84,0,60,"");
  ...
  for(std::vector<int>::iterator run = runs.begin(); run != runs.end(); run++) {
    ...
    Double_t tilt = gettilt(inputdir,*run,chip,"-threshold");
    ...
    //if(*run == 20385 || *run == 20386) threshold = 123;
    ...
    cout << "run" << *run << " (chip" << chip << ") res " << res << " ressub " << res_tel_subtracted << " res_corr " << res_corr_tel_subtracted << " tilt " << tilt << " threshold " << threshold << endl;
    //cout << *run << " " << tilt << " " << res_tel_subtracted << endl;
    //cout << " -> dz = " << dz << ", sigma_tel = " << tel_resolution << endl;

    resolution->Fill(threshold,res,1);
    resolution_corr->Fill(threshold,res_corr,1);

    resolution_tel_subtracted->Fill(threshold,res_tel_subtracted,1);
    resolution_corr_tel_subtracted->Fill(threshold,res_corr_tel_subtracted,1);
  ...
  c1->cd();
  resolution->SetTitle(";threshold [vcal];resolution x #left[#mum#right]");
  ...
  c2->cd();
  resolution_tel_subtracted->SetTitle(";threshold [vcal];resolution x #left[#mum#right]");
